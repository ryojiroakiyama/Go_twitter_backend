...
// 問題点:
//  1. テスト以外でもコンパイルされるという無駄がある
//  2. DBをmapを使ってモックしてみたが, テストを作る上でコストが高すぎる気がする
//     DBのように動く機能を用意せずに, 毎テスト期待する挙動をベタ書きしてみたが大変(でもこれが主流かも)
//     今回は一つの関数内で複数回同じ機能を使用したりしているので, 呼び出されるごとに挙動を変えるような動きをつけたかった
//      -> そういうメソッドを作ればいけるから挙動の定義でもできないことはない
// 改善点:
//  各テストパッケージではその中でテスト処理を完結させる(他テストパッケージをimportしない)
//  テストパッケージが大きくなるようなコードを書かない(今回だとhandler/accountとかをusecase的な機能とさらに分けるべき?)
//  handler系のパッケージはhandlerテストパッケージでテストすべき
// 留意点:
//  _test.goのみで構成されたディレクトリ, パッケージは作れない(テスト対象パッケージがあってのテストパッケージ)
//  _testパッケージはimportのやり方が(今の所)分からない
// 疑問点:
//  複数のテストパッケージで共有したい機能をテスト時のみコンパイルするような形で実現することは可能か？
//  DBのモックの例を色々知りたい
// その他改善点:
//  テストが追加しやすくなったり可読性上がると思って, 各テストファイルの内容はなるべくそのファイル内で書くようにしたが,
//  やっぱり被る機能はまとめた方が後の編集が楽.
